...
ENV_HTTP_RETRIES = "INFERADAR_LLM_HTTP_RETRIES"
ENV_RETRY_BASE_SECONDS = "INFERADAR_LLM_RETRY_BASE_SECONDS"
...
EMPTY_AUTH_PREFIX_SENTINEL = "__EMPTY__"
...
SOURCE_HASH_RE = re.compile(r"<!-- inferadar-source-sha256: ([0-9a-f]{64}) -->")
...
def _footer(display_name: str, source_sha256: str | None = None) -> str:
    source_marker = (
        f"\n<!-- inferadar-source-sha256: {source_sha256} -->"
        if source_sha256
        else ""
    )
    return (
        ...
        f"This file mentions no users and notifies no PRs._{source_marker}\n"
    )
...
def build_markdown(
    ...
    *,
    ...
    body = _ensure_more_changes_heading(body.strip())
    body = linkify_pr_refs(strip_mentions(body.strip()), artifact)
    return _header(artifact, display_name) + body + _footer(display_name, source_sha256)


def _ensure_more_changes_heading(body: str) -> str:
    """Repair a common model omission while preserving the generated prose."""
    heading = "## More changes by area"
    if "<details>" in body and heading not in body:
        return body.replace("<details>", f"{heading}\n\n<details>", 1)
    return body
...
def _auth_headers(api_key: str) -> dict[str, str]:
    ...
    if prefix == EMPTY_AUTH_PREFIX_SENTINEL:
        prefix = ""
...
def call_llm(
    ...
    *,
    ...
    http_retries = max(0, _env_int(ENV_HTTP_RETRIES, DEFAULT_HTTP_RETRIES))
    retry_base = max(0.0, _env_float(ENV_RETRY_BASE_SECONDS, DEFAULT_RETRY_BASE_SECONDS))
    ...
    for attempt in range(empty_retries + 1):
        ...
        for http_attempt in range(http_retries + 1):
            try:
                ...
                if http_attempt >= http_retries:
                    raise
                delay = min(retry_base * (2**http_attempt), MAX_RETRY_DELAY_SECONDS)
                time.sleep(delay)
        ...
        if not last_error.budget_related:
            break
        if attempt >= empty_retries:
            break
        # Escalate toward the cap; once at the cap this is a no-op and we simply
        # retry at max budget (reasoning models emit transient empties).
...
class _TransientGatewayError(RuntimeError):
    """A network or HTTP failure that is reasonable to retry shortly."""
...
def _chat_once(
    ...
    try:
        ...
        if status in {408, 409, 425, 429} or status >= 500:
            raise _TransientGatewayError(f"transient gateway HTTP {status}") from exc
        raise
    except httpx.RequestError as exc:
        raise _TransientGatewayError(f"transient gateway network error: {exc}") from exc
...
def select_window_dirs(
    ...
    if since:
        dirs = [path for path in dirs if path.name[:10] >= since]
    if window == "all":
        return dirs
    if window == "latest":
        return dirs[-1:]
...
def find_target_jsons(
    ...
    *,
    ...
    targets: list[Path] = []
    for window_dir in select_window_dirs(changelogs_dir, window, start, end, since):
...
def summarize_artifact_file(
    ...
    *,
    ...
    if not force and _markdown_matches_source(md_path, source_sha256, artifact):
...
def _markdown_matches_source(
    ...
    try:
        ...
        return False
    marker = SOURCE_HASH_RE.search(markdown)
    if marker:
        return marker.group(1) == source_sha256
    # Digests generated before source fingerprints were introduced include the
    # JSON's generated_at value in their header. Treat those as current when it
    # still matches, avoiding a costly rewrite of historical LLM prose.
    generated_at = str(artifact.get("generated_at", "")).strip()
    return bool(generated_at and f", generated {generated_at}_" in markdown)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--since",
        help="Only consider windows whose start date is on/after YYYY-MM-DD",
    )
